...
```python
pearson_r, p_value = ss.pearsonr(df_pokemon['Attack'], df_pokemon['Defense'])
...
Answer:  
One limitation of Pearson's r is that it only captures *linear* relationships. If the relationship between two variables is nonlinear (e.g., curved), Pearson's r may be close to 0 even if there is a strong association.  

Additionally, Pearson's r is sensitive to outliers, which can significantly distort the correlation coefficient. It also assumes that the variables are normally distributed and measured on an interval or ratio scale.
...
numeric_cols = df_pokemon.select_dtypes(include='number').columns

pearson_corr = df_pokemon[numeric_cols].corr(method='pearson')
spearman_corr = df_pokemon[numeric_cols].corr(method='spearman')
kendall_corr = df_pokemon[numeric_cols].corr(method='kendall')

plt.figure(figsize=(10, 8))
sns.heatmap(pearson_corr, annot=True, cmap='coolwarm', fmt=".2f")
plt.title("Pearson Correlation Matrix (Numeric Pokémon Stats)")
plt.show()
...
To visualize relationships between different types of variables:

- Quantitative vs. Quantitative → Scatter plots  
  Example: Attack vs. Defense

- Quantitative vs. Qualitative → Box plots or Violin plots  
  Example: Speed (quant) vs. Legendary (qualitative: True/False)

- Ranked (Ordinal) vs. Quantitative → Strip plots or Box plots with ordering  
  If we rank Pokémon types by average HP, we could show this using ordered bar plots.

- Qualitative vs. Qualitative → Grouped bar charts or use Cramér’s V to measure association  
  Example: Type 1 vs. Legendary as shown using a contingency table.
