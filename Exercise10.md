...
plt.title('Relation entre nombre de supérettes et prix au m²')
plt.xlabel('n_convenience')
plt.ylabel('price_twd_msq')
plt.grid(True)
plt.show()

sns.regplot(data=df_estate, x='n_convenience', y='price_twd_msq', ci=None)
...
plt.title('Relation entre l\'âge du logement et le prix au m²')
plt.xlabel('Âge du logement (années)')
plt.ylabel('Prix au m² (TWD)')
plt.grid(True)
plt.show()

sns.regplot(data=df_estate, x='house_age_years', y='price_twd_msq', ci=None)
...
plt.title('Distance à la station MRT vs. Prix au m²')
plt.xlabel('Distance à la station MRT (m)')
plt.ylabel('Prix au m² (TWD)')
plt.grid(True)
plt.show()

sns.regplot(data=df_estate, x='dist_to_mrt_m', y='price_twd_msq', ci=None)
...
```python
g = sns.FacetGrid(df_estate, col='house_age_cat', col_wrap=3, height=4)
g.map(plt.hist, 'price_twd_msq', bins=10, edgecolor='black')

g.set_titles('House age group: {col_name}')
g.set_axis_labels('Price per square meter (TWD)', 'Number of properties')
plt.subplots_adjust(top=0.85)
g.fig.suptitle('Histogram of price per square meter by house age group')
plt.show()
...
```python
summary_stats = df_estate.groupby('house_age_cat')['price_twd_msq'].agg(
    mean_price='mean',
    std_price='std',
    median_price='median',
    min_price='min',
...
```
    ...
    C:\Users\maxen\AppData\Local\Temp\ipykernel_63992\3889267274.py:1: FutureWarning: The default of observed=False is deprecated and will be changed to True in a future version of pandas. Pass observed=False to retain current behavior or observed=True to adopt the future default and silence this warning.
      summary_stats = df_estate.groupby('house_age_cat')['price_twd_msq'].agg(
...
```
    ...
    Time:                        23:25:28   Log-Likelihood:                -1542.5
...
<tr>
  <th>Time:</th>                 <td>23:25:31</td>     <th>  Log-Likelihood:    </th> <td> -1530.2</td>
</tr>
<tr>
...
plt.figure(figsize=(8, 6))
sns.scatterplot(data=df_estate, x='dist_to_mrt_m', y='price_twd_msq')
plt.title('Scatter plot of distance to MRT vs. price per square meter')
...
```python
df_estate['log_dist_to_mrt_m'] = np.log1p(df_estate['dist_to_mrt_m'])

plt.figure(figsize=(8, 6))
sns.scatterplot(data=df_estate, x='log_dist_to_mrt_m', y='price_twd_msq')
plt.title('Log-transformed distance to MRT vs. price per square meter')
...
model = smf.ols('price_twd_msq ~ dist_to_mrt_m + house_age_0_15 + house_age_30_45', data=df_estate)
result = model.fit()
...
```
    ...
    Time:                        23:28:02   Log-Likelihood:                -1530.2
...
```
    ...
    Time:                        23:25:33   Log-Likelihood:                -1525.6
...
```python
model_refined = smf.ols('price_twd_msq ~ dist_to_mrt_m + house_age_0_15', data=df_estate)
...
from statsmodels.stats.outliers_influence import variance_inflation_factor
X = result_refined.model.exog
vif = [variance_inflation_factor(X, i) for i in range(X.shape[1])]

for name, val in zip(result_refined.model.exog_names, vif):
    print(f"VIF for {name}: {val:.2f}")
...
![png](output_63_2.png)
    ...
    VIF for Intercept: 2.80
    VIF for dist_to_mrt_m: 1.01
    VIF for house_age_0_15: 1.01
